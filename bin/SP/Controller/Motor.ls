...
function Motor:MoveXYZ(xyzpos, mode)
    self.isRunning = true;
    return self.motorControlInterface:MotorMoveXYZ(xyzpos, mode)
end
...
function Motor:ExpectResult(timeout)
    ...
    --motorResult = self.peristalticPumpInterface:ExpectPumpResult(timeout)
