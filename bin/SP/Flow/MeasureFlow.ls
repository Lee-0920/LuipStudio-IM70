...
MeasureFlow = Flow:new
{
    ...
    isOverProof = false,
    measureChannelOne = false,
    measureChannelTwo = false,
    measureChannelThree = false,
    measureChannelFour = false,
    measureChannelFive = false,
    measureChannelSix = false,
    valueTC = 0,
    valueIC = 0,
    valueNPOC = 0,
    valueTOC = 0,
...
function MeasureFlow:new(o, meaType)
    ...
    o.currentView = config.system.rangeViewMap[config.measureParam.range[config.measureParam.currentRange + 1] + 1].view
    o.isOverProof = false
    o.measureCheck = false
    o.measureChannelOne = false
    o.measureChannelTwo = false
    o.measureChannelThree = false
    o.measureChannelFour = false
    o.measureChannelFive = false
    o.measureChannelSix = false
    o.valueTC = 0
    o.valueIC = 0
    o.valueNPOC = 0
    o.valueTOC = 0
...
function MeasureFlow:OnStart()
    ...
    if self.measureType ~= MeasureType.Sample then  --非水样数据
        self.measureCheck = true
    else
        if config.system.targetMap.ChannelOne.enable and self.target == config.system.targetMap.ChannelOne.num then
            self.measureChannelOne = true
            self.currentRange = config.measureParam.channelRangeIndex[1] + 1
        end
        if config.system.targetMap.ChannelTwo.enable and self.target == config.system.targetMap.ChannelTwo.num then
            self.measureChannelTwo = true
            self.currentRange = config.measureParam.channelRangeIndex[2] + 1
        end
        if config.system.targetMap.ChannelThree.enable and self.target == config.system.targetMap.ChannelThree.num then
            self.measureChannelThree = true
            self.currentRange = config.measureParam.channelRangeIndex[3] + 1
        end
        if config.system.targetMap.ChannelFour.enable and self.target == config.system.targetMap.ChannelFour.num then
            self.measureChannelFour = true
            self.currentRange = config.measureParam.channelRangeIndex[4] + 1
        end
        if config.system.targetMap.ChannelFive.enable and self.target == config.system.targetMap.ChannelFive.num then
            self.measureChannelFive = true
            self.currentRange = config.measureParam.channelRangeIndex[5] + 1
        end
        if config.system.targetMap.ChannelSix.enable and self.target == config.system.targetMap.ChannelSix.num then
            self.measureChannelSix = true
            self.currentRange = config.measureParam.channelRangeIndex[6] + 1
        end
        self.currentView = config.system.rangeViewMap[self.currentRange].view
    end
    ...
    elseif self.measureType == MeasureType.Sample then   --测水样
        ...
        if self.measureChannelTwo then
            runStatus = Helper.Status.SetStatus(setting.runStatus.measureSample2)
        elseif self.measureChannelThree then
            runStatus = Helper.Status.SetStatus(setting.runStatus.measureSample3)
        elseif self.measureChannelFour then
            runStatus = Helper.Status.SetStatus(setting.runStatus.measureSample4)
        elseif self.measureChannelFive then
            runStatus = Helper.Status.SetStatus(setting.runStatus.measureSample5)
        elseif self.measureChannelSix then
            runStatus = Helper.Status.SetStatus(setting.runStatus.measureSample6)
        end
        StatusManager.Instance():SetStatus(runStatus)
    ...
    while self.isRetryMeasure == true and loopcnt > 0 do
        ...
        local err,result = pcall(			-- 捕获异常
            function()
                ...
                if  self.measureChannelOne then
                    Measurer.addParam.supplyChannel = setting.liquidType.exSampleIn1
                    Measurer.addParam.sampleChannel = setting.liquidType.exSampleOut1
                    setting.measureResult.mode = config.scheduler.measure.mode
                elseif self.measureChannelTwo then
                    Measurer.addParam.supplyChannel = setting.liquidType.exSampleIn2
                    Measurer.addParam.sampleChannel = setting.liquidType.exSampleOut2
                    setting.measureResult.mode = config.scheduler.measureChannelTwo.mode
                elseif self.measureChannelThree then
                    Measurer.addParam.supplyChannel = setting.liquidType.exSampleIn3
                    Measurer.addParam.sampleChannel = setting.liquidType.exSampleOut3
                    setting.measureResult.mode = config.scheduler.measureChannelThree.mode
                elseif self.measureChannelFour then
                    Measurer.addParam.supplyChannel = setting.liquidType.exSampleIn4
                    Measurer.addParam.sampleChannel = setting.liquidType.exSampleOut4
                    setting.measureResult.mode = config.scheduler.measureChannelFour.mode
                elseif self.measureChannelFive then
                    Measurer.addParam.supplyChannel = setting.liquidType.exSampleIn5
                    Measurer.addParam.sampleChannel = setting.liquidType.exSampleOut5
                    setting.measureResult.mode = config.scheduler.measureChannelFive.mode
                elseif self.measureChannelSix then
                    Measurer.addParam.supplyChannel = setting.liquidType.exSampleIn6
                    Measurer.addParam.sampleChannel = setting.liquidType.exSampleOut6
                    setting.measureResult.mode = config.scheduler.measureChannelSix.mode
                end
                Measurer.currentModel = config.measureParam.measureType

                self.currentModelType = Measurer.currentModel

                log:debug("当前量程 = " .. self.currentRange)
                log:debug("当前测量参数：" .. self:GetModel(self.currentModelType))
                if nil ~= setting.common.skipFlow and true == setting.common.skipFlow then
                ...
                if self.isCrashMeasure == true and config.system.CCEPMode == true then
                    ...
                    setting.measureResult.isStartCalibrate = false          --校准流程
                    ...
                    if setting.measureResult.mode == MeasureMode.Continous and self.measureType == MeasureType.Sample then
                        while setting.measureResult.mode == MeasureMode.Continous and self.measureType == MeasureType.Sample do
                            --连续模式
                            ...
                            --量程切换
                            if  self.measureChannelOne then
                                if self.currentRange ~= config.measureParam.channelRangeIndex[1] + 1 and setting.measureResult.continousModeParam.isfinishContinousMeasure == true then
                                    log:debug("channel 1 change range")
                                    Measurer:ContinousMeasureSafetyStop()
                                    self.isFinish = true
                                    break
                                end
                            elseif self.measureChannelTwo then
                                if self.currentRange ~= config.measureParam.channelRangeIndex[2] + 1 and setting.measureResult.continousModeParam.isfinishContinousMeasure == true then
                                    log:debug("channel 2 change range")
                                    Measurer:ContinousMeasureSafetyStop()
                                    self.isFinish = true
                                    break
                                end
                            elseif self.measureChannelThree then
                                if self.currentRange ~= config.measureParam.channelRangeIndex[3] + 1 and setting.measureResult.continousModeParam.isfinishContinousMeasure == true then
                                    log:debug("channel 3 change range")
                                    Measurer:ContinousMeasureSafetyStop()
                                    self.isFinish = true
                                    break
                                end
                            elseif self.measureChannelFour then
                                if self.currentRange ~= config.measureParam.channelRangeIndex[4] + 1 and setting.measureResult.continousModeParam.isfinishContinousMeasure == true then
                                    log:debug("channel 4 change range")
                                    Measurer:ContinousMeasureSafetyStop()
                                    self.isFinish = true
                                    break
                                end
                            elseif self.measureChannelFive then
                                if self.currentRange ~= config.measureParam.channelRangeIndex[5] + 1 and setting.measureResult.continousModeParam.isfinishContinousMeasure == true then
                                    log:debug("channel 5 change range")
                                    Measurer:ContinousMeasureSafetyStop()
                                    self.isFinish = true
                                    break
                                end
                            elseif self.measureChannelSix then
                                if self.currentRange ~= config.measureParam.channelRangeIndex[6] + 1 and setting.measureResult.continousModeParam.isfinishContinousMeasure == true then
                                    log:debug("channel 6 change range")
                                    Measurer:ContinousMeasureSafetyStop()
                                    self.isFinish = true
                                    break
                                end
                            ...
                            if config.scheduler.drainFromRefrigerator.enable == true and setting.measureResult.continousModeParam.isfinishContinousMeasure == true then --避免重复出数
                                --自动排冷凝液排期
                                local currentTime = os.time()
                                local ret, actionTime = setting.measureScheduler[13].getNextTime()
                                if currentTime - actionTime >= 0 then
                                    log:debug("drainFromRefrigerator")
                                    Measurer:ContinousMeasureSafetyStop()
                                    self.isFinish = true
                                    break
                                end
                            ...
                            if config.scheduler.standard.enable == true and setting.measureResult.continousModeParam.isfinishContinousMeasure == true then
                                --量程校准排期
                                local currentTime = os.time()
                                local ret, actionTime = setting.measureScheduler[10].getNextTime()
                                if currentTime - actionTime >= 0 then
                                    log:debug("standard")
                                    Measurer:ContinousMeasureSafetyStop()
                                    self.isFinish = true
                            ...
                            if config.scheduler.blankCheck.enable == true and setting.measureResult.continousModeParam.isfinishContinousMeasure == true then
                                --零点校准排期
                                local currentTime = os.time()
                                local ret, actionTime = setting.measureScheduler[12].getNextTime()
                                if currentTime - actionTime >= 0 then
    ...
    if not self.isFinish then
        if self.measureType == MeasureType.Sample then
            if self.measureChannelOne then
                ...
                status.measure.schedule.autoMeasureChannelOne.dateTime = self.measureDateTime
            elseif self.measureChannelTwo then
                status.measure.schedule.autoMeasureChannelTwo.dateTime = self.measureDateTime
            elseif self.measureChannelThree then
                status.measure.schedule.autoMeasureChannelThree.dateTime = self.measureDateTime
            elseif self.measureChannelFour then
                status.measure.schedule.autoMeasureChannelFour.dateTime = self.measureDateTime
            elseif self.measureChannelFive then
                status.measure.schedule.autoMeasureChannelFive.dateTime = self.measureDateTime
            elseif self.measureChannelSix then
                status.measure.schedule.autoMeasureChannelSix.dateTime = self.measureDateTime
            end
        ...
        if self.isUserStop then
            ...
            if self.measureType == MeasureType.Sample then
                ...
                if self.measureChannelOne and config.scheduler.measure.mode == MeasureMode.Continous then
                    config.scheduler.measure.mode = MeasureMode.Trigger --连续测量变触发
                elseif self.measureChannelTwo and config.scheduler.measureChannelTwo.mode == MeasureMode.Continous then
                    config.scheduler.measureChannelTwo.mode = MeasureMode.Trigger --连续测量变触发
                elseif self.measureChannelThree and config.scheduler.measureChannelThree.mode == MeasureMode.Continous  then
                    config.scheduler.measureChannelThree.mode = MeasureMode.Trigger --连续测量变触发
                elseif self.measureChannelFour and config.scheduler.measureChannelFour.mode == MeasureMode.Continous  then
                    config.scheduler.measureChannelFour.mode = MeasureMode.Trigger --连续测量变触发
                elseif self.measureChannelFive and config.scheduler.measureChannelFive.mode == MeasureMode.Continous  then
                    config.scheduler.measureChannelFive.mode = MeasureMode.Trigger --连续测量变触发
                elseif self.measureChannelSix and config.scheduler.measureChannelSix.mode == MeasureMode.Continous  then
                    config.scheduler.measureChannelSix.mode = MeasureMode.Trigger --连续测量变触发
                end
                isSave = true
...
function MeasureFlow:ResultHandle(result)
    ...
    if setting.measureResult.mode == MeasureMode.Continous and setting.measureResult.continousModeParam.currentMeasureCnt ~= 0 then
    ...
    elseif setting.measureResult.mode ~= MeasureMode.Continous and setting.measureResult.continousModeParam.currentMeasureCnt ~= 0 then
    ...
    --if self.currentModelType == ModelType.TOC or self.currentModelType == ModelType.CODT then
    --    self.measureDateTime = result.measureDate
    --end
    ...
    --异常吸光度修正
    self.peakAreaIC = self:AbnormalAbsorbanceRevise(peakAreaIC)
   ...
   --浓度计算  当前result.peak 应避免未空值
    if nil ~= setting.common.skipFlow and true == setting.common.skipFlow then
        ...
        if self.currentModelType == ModelType.TC or self.currentModelType == ModelType.NPOC or self.currentModelType == ModelType.CODN then
            self.peakArea = self:ReviseArea(self.peakArea, self.currentModelType)
            self.consistency = self:CalculateConsistency(self.peakArea, self.currentModelType)
            if self.currentModelType == ModelType.CODN then
                self.valueNPOC = self.consistency
            end
        elseif self.currentModelType == ModelType.IC then
            self.peakAreaIC = self:ReviseArea(self.peakAreaIC, self.currentModelType)
            self.peakArea =  self.peakAreaIC
            self.consistency = self:CalculateConsistency(self.peakAreaIC, self.currentModelType)
        elseif self.currentModelType == ModelType.TOC or self.currentModelType == ModelType.CODT then
            self.peakArea = self:ReviseArea(self.peakArea, ModelType.TC)
            self.peakAreaIC = self:ReviseArea(self.peakAreaIC, ModelType.IC)
            local consistencyIC = self:CalculateConsistency(self.peakAreaIC, ModelType.IC)
            local consistencyTC = self:CalculateConsistency(self.peakArea, ModelType.TC)
            self.valueIC = self.consistencyIC
            self.valueTC = self.consistencyTC
            self.peakArea = 0
            self.consistency = consistencyTC - consistencyIC
            log:debug("TC测量值 = " .. consistencyTC .. ", IC测量值 = " .. consistencyIC)
        end
    end

    --内部系数校正
    self.consistency = self:ReviseParameter(self.consistency, self.currentRange)
    ...
    if self.measureCheck then
        if self.currentModelType == ModelType.CODT or self.currentModelType == ModelType.CODN then
            self.consistency = self.consistency * config.system.channelFactor[7].curveK + config.system.channelFactor[7].curveB
        end
        --整点测量时，测量时间整点校正，手动启动不校正
        if self.adjustTime == true then  --排期设置
            if (self.measureType == MeasureType.Sample and  setting.measureResult.mode == MeasureMode.Timed) or
        ...
        recordData:PushFloat(string.format("%.2f", self.valueIC)) -- IC测量值
        recordData:PushFloat(string.format("%.2f", self.valueTC)) -- TC测量值
        recordData:PushFloat(string.format("%.2f", self.valueNPOC)) -- NPOC测量值
        recordData:PushFloat(string.format("%.2f", config.system.channelFactor[7].curveK)) -- 转换系数
    ...
    if self.measureChannelOne then
        if self.currentModelType == ModelType.CODT or self.currentModelType == ModelType.CODN then
            self.consistency = self.consistency * config.system.channelFactor[1].curveK + config.system.channelFactor[1].curveB
        end
        --整点测量时，测量时间整点校正，手动启动不校正
        if self.adjustTime == true then  --排期设置
            if (self.measureType == MeasureType.Sample and  setting.measureResult.mode == MeasureMode.Timed) or
        ...
        --print("Push result data to file.")
        local resultManager = ResultManager.Instance()
        local recordData = RecordData.new(resultManager:GetMeasureRecordDataSize(setting.resultFileInfo.measureRecordFile[2].name))
        recordData:PushInt(self.measureDateTime) -- 时间
        recordData:PushFloat(self.consistency) -- 浓度
        recordData:PushFloat(self.peakArea) -- 峰面积
        recordData:PushByte(resultMark) -- 结果标识
        ...
        recordData:PushFloat(string.format("%.2f", self.valueTC)) -- TC测量值
        recordData:PushFloat(string.format("%.2f", self.valueNPOC)) -- NPOC测量值
        recordData:PushFloat(string.format("%.2f", config.system.channelFactor[1].curveK)) -- 转换系数

        local flowManager = FlowManager.Instance()
        if true == flowManager:IsReagentAuthorize() then
            Helper.Result.OnMeasureResultAddedChannelOne(Measurer.measureType, self.measureDateTime, self.consistency, self.reportMode, self.peakArea ,self.isUseStart,false,resultMark,modbusStr, self.currentModelType)
            resultManager:AddMeasureRecord(setting.resultFileInfo.measureRecordFile[2].name, recordData, true)
    ...
    if self.measureChannelTwo then
        if self.currentModelType == ModelType.CODT or self.currentModelType == ModelType.CODN then
            self.consistency = self.consistency * config.system.channelFactor[2].curveK + config.system.channelFactor[2].curveB
        end
        --整点测量时，测量时间整点校正，手动启动不校正
        if self.adjustTime == true then  --排期设置
            if (self.measureType == MeasureType.Sample and  setting.measureResult.mode == MeasureMode.Timed) or
        ...
        --print("Push result data to file.")
        local resultManager = ResultManager.Instance()
        local recordData = RecordData.new(resultManager:GetMeasureRecordDataSize(setting.resultFileInfo.measureRecordFile[3].name))
        recordData:PushInt(self.measureDateTime) -- 时间
        recordData:PushFloat(self.consistency) -- 浓度
        recordData:PushFloat(self.peakArea) -- 峰面积
        recordData:PushByte(resultMark) -- 结果标识
        ...
        recordData:PushFloat(string.format("%.2f", self.valueTC)) -- TC测量值
        recordData:PushFloat(string.format("%.2f", self.valueNPOC)) -- NPOC测量值
        recordData:PushFloat(string.format("%.2f", config.system.channelFactor[2].curveK)) -- 转换系数

        local flowManager = FlowManager.Instance()
        if true == flowManager:IsReagentAuthorize() then
            Helper.Result.OnMeasureResultAddedChannelTwo(Measurer.measureType, self.measureDateTime, self.consistency, self.reportMode, self.peakArea ,self.isUseStart,false,resultMark,modbusStr, self.currentModelType)
            resultManager:AddMeasureRecord(setting.resultFileInfo.measureRecordFile[3].name, recordData, true)
    ...
    if self.measureChannelThree then
        if self.currentModelType == ModelType.CODT or self.currentModelType == ModelType.CODN then
            self.consistency = self.consistency * config.system.channelFactor[3].curveK + config.system.channelFactor[3].curveB
        end
        --整点测量时，测量时间整点校正，手动启动不校正
        if self.adjustTime == true then  --排期设置
            if (self.measureType == MeasureType.Sample and  setting.measureResult.mode == MeasureMode.Timed) or
        ...
        --print("Push result data to file.")
        local resultManager = ResultManager.Instance()
        local recordData = RecordData.new(resultManager:GetMeasureRecordDataSize(setting.resultFileInfo.measureRecordFile[4].name))
        recordData:PushInt(self.measureDateTime) -- 时间
        recordData:PushFloat(self.consistency) -- 浓度
        recordData:PushFloat(self.peakArea) -- 峰面积
        recordData:PushByte(resultMark) -- 结果标识
        ...
        recordData:PushFloat(string.format("%.2f", self.valueTC)) -- TC测量值
        recordData:PushFloat(string.format("%.2f", self.valueNPOC)) -- NPOC测量值
        recordData:PushFloat(string.format("%.2f", config.system.channelFactor[3].curveK)) -- 转换系数

        local flowManager = FlowManager.Instance()
        if true == flowManager:IsReagentAuthorize() then
            Helper.Result.OnMeasureResultAddedChannelThree(Measurer.measureType, self.measureDateTime, self.consistency, self.reportMode, self.peakArea ,self.isUseStart,false,resultMark,modbusStr, self.currentModelType)
            resultManager:AddMeasureRecord(setting.resultFileInfo.measureRecordFile[4].name, recordData, true)
    ...
    if self.measureChannelFour then
        if self.currentModelType == ModelType.CODT or self.currentModelType == ModelType.CODN then
            self.consistency = self.consistency * config.system.channelFactor[4].curveK + config.system.channelFactor[4].curveB
        end
        --整点测量时，测量时间整点校正，手动启动不校正
        if self.adjustTime == true then  --排期设置
            if (self.measureType == MeasureType.Sample and  setting.measureResult.mode == MeasureMode.Timed) or
        ...
        --print("Push result data to file.")
        local resultManager = ResultManager.Instance()
        local recordData = RecordData.new(resultManager:GetMeasureRecordDataSize(setting.resultFileInfo.measureRecordFile[5].name))
        recordData:PushInt(self.measureDateTime) -- 时间
        recordData:PushFloat(self.consistency) -- 浓度
        recordData:PushFloat(self.peakArea) -- 峰面积
        recordData:PushByte(resultMark) -- 结果标识
        ...
        recordData:PushFloat(string.format("%.2f", self.valueTC)) -- TC测量值
        recordData:PushFloat(string.format("%.2f", self.valueNPOC)) -- NPOC测量值
        recordData:PushFloat(string.format("%.2f", config.system.channelFactor[4].curveK)) -- 转换系数

        local flowManager = FlowManager.Instance()
        if true == flowManager:IsReagentAuthorize() then
            Helper.Result.OnMeasureResultAddedChannelFour(Measurer.measureType, self.measureDateTime, self.consistency, self.reportMode, self.peakArea ,self.isUseStart,false,resultMark,modbusStr, self.currentModelType)
            resultManager:AddMeasureRecord(setting.resultFileInfo.measureRecordFile[5].name, recordData, true)
    ...
    if self.measureChannelFive then
        if self.currentModelType == ModelType.CODT or self.currentModelType == ModelType.CODN then
            self.consistency = self.consistency * config.system.channelFactor[5].curveK + config.system.channelFactor[5].curveB
        end
        --整点测量时，测量时间整点校正，手动启动不校正
        if self.adjustTime == true then  --排期设置
            if (self.measureType == MeasureType.Sample and  setting.measureResult.mode == MeasureMode.Timed) or
        ...
        --print("Push result data to file.")
        local resultManager = ResultManager.Instance()
        local recordData = RecordData.new(resultManager:GetMeasureRecordDataSize(setting.resultFileInfo.measureRecordFile[6].name))
        recordData:PushInt(self.measureDateTime) -- 时间
        recordData:PushFloat(self.consistency) -- 浓度
        recordData:PushFloat(self.peakArea) -- 峰面积
        recordData:PushByte(resultMark) -- 结果标识
        ...
        recordData:PushFloat(string.format("%.2f", self.valueTC)) -- TC测量值
        recordData:PushFloat(string.format("%.2f", self.valueNPOC)) -- NPOC测量值
        recordData:PushFloat(string.format("%.2f", config.system.channelFactor[5].curveK)) -- 转换系数

        local flowManager = FlowManager.Instance()
        if true == flowManager:IsReagentAuthorize() then
            Helper.Result.OnMeasureResultAddedChannelFive(Measurer.measureType, self.measureDateTime, self.consistency, self.reportMode, self.peakArea ,self.isUseStart,false,resultMark,modbusStr, self.currentModelType)
            resultManager:AddMeasureRecord(setting.resultFileInfo.measureRecordFile[6].name, recordData, true)
    ...
    if self.measureChannelSix then
        if self.currentModelType == ModelType.CODT or self.currentModelType == ModelType.CODN then
            self.consistency = self.consistency * config.system.channelFactor[6].curveK + config.system.channelFactor[6].curveB
        end
        --整点测量时，测量时间整点校正，手动启动不校正
        if self.adjustTime == true then  --排期设置
            if (self.measureType == MeasureType.Sample and  setting.measureResult.mode == MeasureMode.Timed) or
        ...
        --print("Push result data to file.")
        local resultManager = ResultManager.Instance()
        local recordData = RecordData.new(resultManager:GetMeasureRecordDataSize(setting.resultFileInfo.measureRecordFile[7].name))
        recordData:PushInt(self.measureDateTime) -- 时间
        recordData:PushFloat(self.consistency) -- 浓度
        recordData:PushFloat(self.peakArea) -- 峰面积
        recordData:PushByte(resultMark) -- 结果标识
        ...
        recordData:PushFloat(string.format("%.2f", self.valueTC)) -- TC测量值
        recordData:PushFloat(string.format("%.2f", self.valueNPOC)) -- NPOC测量值
        recordData:PushFloat(string.format("%.2f", config.system.channelFactor[6].curveK)) -- 转换系数

        local flowManager = FlowManager.Instance()
        if true == flowManager:IsReagentAuthorize() then
            Helper.Result.OnMeasureResultAddedChannelSix(Measurer.measureType, self.measureDateTime, self.consistency, self.reportMode, self.peakArea ,self.isUseStart,false,resultMark,modbusStr, self.currentModelType)
            resultManager:AddMeasureRecord(setting.resultFileInfo.measureRecordFile[7].name, recordData, true)
    ...
    if true == self.isChangeRangeAtTheEnd then
        if self.measureChannelOne then
            config.measureParam.channelRangeIndex[1] = self.targetRange 		--更新量程配置文件
        elseif self.measureChannelTwo then
            config.measureParam.channelRangeIndex[2] = self.targetRange 		--更新量程配置文件
        elseif self.measureChannelThree then
            config.measureParam.channelRangeIndex[3] = self.targetRange 		--更新量程配置文件
        elseif self.measureChannelFour then
            config.measureParam.channelRangeIndex[4] = self.targetRange 		--更新量程配置文件
        elseif self.measureChannelFive then
            config.measureParam.channelRangeIndex[5] = self.targetRange 		--更新量程配置文件
        elseif self.measureChannelSix then
            config.measureParam.channelRangeIndex[6] = self.targetRange 		--更新量程配置文件
        end
...
--]]
function MeasureFlow:CalculateConsistency(area, modelType)
    local consistency = 0
    local curveK = config.measureParam.curveParam[1].curveK
    local curveB = config.measureParam.curveParam[1].curveB
    local diluteFactor = setting.measure.range[self.currentRange].diluteFactor

    if modelType == ModelType.IC then
        curveK = config.measureParam.curveParam[1].curveKIC
        curveB = config.measureParam.curveParam[1].curveBIC
    elseif modelType == ModelType.NPOC or modelType == ModelType.CODN then
        curveK = config.measureParam.curveParam[1].curveKNPOC
        curveB = config.measureParam.curveParam[1].curveBNPOC
    end
    ...
    log:debug("量程"..self.currentRange.."计算斜率 K = " .. curveK .. ", B = " .. curveB .. ", 稀释系数 = " .. diluteFactor)

    consistency = (area - curveB)/curveK
    consistency = consistency * diluteFactor

    if config.measureParam.absCalibrateMode == true then
        consistency = consistency + curveB
        log:debug("绝对浓度计算模式 = " .. consistency)
    end
...
--]]
function MeasureFlow:ReviseParameter(consistency,range)
    local ret = consistency

    if range ~= nil and range < setting.measure.range.rangeNum then
        ret = config.measureParam.reviseParameter[range] * consistency
        log:debug("内部校正 当前量程校正系数 = " .. config.measureParam.reviseParameter[range] .. ", 原浓度结果 = " .. consistency .. ", 修正结果 = " .. ret)
    end
...
--]]
function MeasureFlow:ReviseArea(area, modelType)
...
function MeasureFlow:RangeCheck(result, currange)
    ...
    elseif currange == setting.measure.range.rangeNum and result > setting.measure.range[currange].viewRange then
        ...
        local currentRange = config.measureParam.channelRangeIndex[1] --默认测量程校准液/测核查
        if self.measureChannelOne then
            currentRange = config.measureParam.channelRangeIndex[1]
        elseif self.measureChannelTwo then
            currentRange = config.measureParam.channelRangeIndex[2]
        elseif self.measureChannelThree then
            currentRange = config.measureParam.channelRangeIndex[3]
        elseif self.measureChannelFour then
            currentRange = config.measureParam.channelRangeIndex[4]
        elseif self.measureChannelFive then
            currentRange = config.measureParam.channelRangeIndex[5]
        elseif self.measureChannelSix then
            currentRange = config.measureParam.channelRangeIndex[6]
        end
        ...
        if self.targetRange ~= currentRange then
            if config.system.rangeViewMap[currentRange + 1].view ~= nil then
                log:info("量程自动切换至 = " .. config.system.rangeViewMap[self.targetRange + 1].view)
            end
        end

        if config.measureParam.activeRangeMode == setting.mode.activeRange.now and self.targetRange ~= currentRange then
            log:info("量程切换：立即生效")
            ...
            ConfigLists.SaveMeasureParamConfig()
            self.currentRange = self.targetRange
            config.modifyRecord.measureParam(true)
            ConfigLists.SaveMeasureParamConfig()
            error(MeasureRangeWrongException:new())  --抛出量程错误异常
        elseif config.measureParam.activeRangeMode == setting.mode.activeRange.next and self.targetRange ~= currentRange then
            log:info("量程切换：下次生效")
            self.isChangeRangeAtTheEnd = true
        elseif self.targetRange == currentRange then
            log:info("量程切换：无需切换")
        end
    end
...
function MeasureFlow:CollectSample()
    ...
    local speed = 1
    if self:CheckTime(self.measureDateTime) and setting.measureResult.mode == MeasureMode.Timed then
        collectAll = true
        self.adjustTime = true
        log:debug("整点测量，所有通道进行水样采集")
    end
    --if nil ~= setting.common.skipFlow and true == setting.common.skipFlow then
    --    return
    --end
    ...
    if collectAll then
        op:DrainExtendSampleStart(setting.liquidType.exSampleIn1, config.measureParam.drainFromMixerVol, speed)
        op:DrainExtendSampleStart(setting.liquidType.exSampleIn2, nil, speed)
        op:DrainExtendSampleStart(setting.liquidType.exSampleIn3, nil, speed)
        op:DrainExtendSampleStart(setting.liquidType.exSampleIn4, nil, speed)
        op:DrainExtendSampleStart(setting.liquidType.exSampleIn6, nil, speed)

        op:DrainExtendSampleStop(setting.liquidType.exSampleIn1, true)
        op:DrainExtendSampleStop(setting.liquidType.exSampleIn2)
        op:DrainExtendSampleStop(setting.liquidType.exSampleIn3)
        op:DrainExtendSampleStop(setting.liquidType.exSampleIn4)
        op:DrainExtendSampleStop(setting.liquidType.exSampleIn6)

        App.Sleep(2000);

        --清空所有泵事件
        dc:ClearPumpRemainEvent()

        op:PumpExtendSampleStart(setting.liquidType.exSampleIn1, config.measureParam.sampleSupplyVolume, speed)
        op:PumpExtendSampleStart(setting.liquidType.exSampleIn2, nil, speed)
        op:PumpExtendSampleStart(setting.liquidType.exSampleIn3, nil, speed)
        op:PumpExtendSampleStart(setting.liquidType.exSampleIn4, nil, speed)
        op:PumpExtendSampleStart(setting.liquidType.exSampleIn6, nil, speed)

        op:PumpExtendSampleStop(setting.liquidType.exSampleIn1, true)
        op:PumpExtendSampleStop(setting.liquidType.exSampleIn2)
        op:PumpExtendSampleStop(setting.liquidType.exSampleIn3)
        op:PumpExtendSampleStop(setting.liquidType.exSampleIn4)
        op:PumpExtendSampleStop(setting.liquidType.exSampleIn6)

        App.Sleep(3000);
        --清空所有泵事件
        dc:ClearPumpRemainEvent()
    elseif setting.measureResult.mode == MeasureMode.Timed and self.isUseStart == false then
        self.adjustTime = true
        log:debug("整点连续测量，不进行水样采集")
    else
        op:DrainExtendSampleStart(Measurer.addParam.supplyChannel, config.measureParam.drainFromMixerVol, speed)
        op:DrainExtendSampleStop(Measurer.addParam.supplyChannel, true)
        --op:PumpExtendSampleStart(Measurer.addParam.sampleChannel, drainVol, speed)
        --op:PumpExtendSampleStop(Measurer.addParam.sampleChannel, true)
        App.Sleep(2000);
        op:PumpExtendSampleStart(Measurer.addParam.supplyChannel, config.measureParam.sampleSupplyVolume, speed)
        op:PumpExtendSampleStop(Measurer.addParam.supplyChannel, true)
    end

    if config.measureParam.stirTime > 0 then
        --打开1-6均质器
        local map = ExValveMap.new(0)
        if collectAll then
            --打开1-6均质器
            map:SetData(setting.liquidType.exSampleIn1.valve | setting.liquidType.exSampleIn2.valve | setting.liquidType.exSampleIn3.valve
                        | setting.liquidType.exSampleIn4.valve | setting.liquidType.exSampleIn6.valve)
        else
            map:SetData(Measurer.addParam.supplyChannel.valve)
        end
        lc:GetIExtSolenoidValve():SetValveMap(map)
        if not Measurer.flow:Wait(config.measureParam.stirTime) then
 ...
 * @details 测量水样时，采集更新水样
--]]
function MeasureFlow:SupplySample()
    local runAction = Helper.Status.SetAction(setting.runAction.collectSample.collect)
    StatusManager.Instance():SetAction(runAction)
    local speed = 1
    local drainVol = config.measureParam.drainFromMixerVol
    if nil ~= setting.common.skipFlow and true == setting.common.skipFlow then
        return
    end

    op:DrainExtendSampleStart(Measurer.addParam.supplyChannel, drainVol, speed)
    op:DrainExtendSampleStop(Measurer.addParam.supplyChannel, true)
    App.Sleep(1000);
    op:PumpExtendSampleStart(Measurer.addParam.supplyChannel, config.measureParam.sampleSupplyVolume, speed)
    op:PumpExtendSampleStop(Measurer.addParam.supplyChannel, true)
    ...
    if config.measureParam.stirTime > 0 then
        --打开1-6均质器
        local map = ExValveMap.new(Measurer.addParam.supplyChannel.valve)
        lc:GetIExtSolenoidValve():SetValveMap(map)
        if not Measurer.flow:Wait(config.measureParam.stirTime) then
            error(UserStopException:new())
        end
...
function MeasureFlow:SafeResultGenerate()
    ...
    local converfactor = 0
    local fileName = setting.resultFileInfo.measureRecordFile[1].name
    if self.measureChannelOne then
        fileName = setting.resultFileInfo.measureRecordFile[2].name
    elseif self.measureChannelTwo then
        fileName = setting.resultFileInfo.measureRecordFile[3].name
    elseif self.measureChannelThree then
        fileName = setting.resultFileInfo.measureRecordFile[4].name
    elseif self.measureChannelFour then
        fileName = setting.resultFileInfo.measureRecordFile[5].name
    elseif self.measureChannelFive then
        fileName = setting.resultFileInfo.measureRecordFile[6].name
    elseif self.measureChannelSix then
        fileName = setting.resultFileInfo.measureRecordFile[7].name
    end
...
--	print("Safe Result Generate.")
    ...
    recordData:PushFloat(string.format("%.2f", self.valueTC)) -- TC测量值
    recordData:PushFloat(string.format("%.2f", self.valueNPOC)) -- NPOC测量值
    recordData:PushFloat(string.format("%.2f", config.system.sampleConverCurveK)) -- 转换系数
...
function MeasureFlow:AdjustMeasureDateTime(meaDataTime)
    ...
    if meaDataTime and meaDataTime >= 0 then
        ...
        elseif minute > 0 and minute <= 50 then
...
--[[
 * @brief 整点测量检查
 * @details 测量时间是0分0秒时，返回true。
--]]
function MeasureFlow:CheckTime(meaDataTime)
    local newTime = meaDataTime
    local ret = false
    local strOldTime = os.date("%Y-%m-%d %H:%M:%S",meaDataTime)
    log:debug("整点测量-原测量时间：" .. strOldTime .. " (" .. meaDataTime .. ")")
...
function MeasureFlow:GetModel(model)
    local str = ""
    if model == ModelType.TC then
        str = "ModelType.TC"
    elseif model == ModelType.IC then
        str = "ModelType.IC"
    elseif model == ModelType.NPOC then
        str = "ModelType.NPOC"
    elseif model == ModelType.TOC then
        str = "ModelType.TOC"
    elseif model == ModelType.CODT then
        str = "ModelType.CODT"
    elseif model == ModelType.CODN then
        str = "ModelType.CODN"
    end
    return str
end
