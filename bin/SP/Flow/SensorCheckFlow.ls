...
function SensorCheckFlow:OnProcess()
    ...
    local err, ret = pcall(function()
                                    ...
                                    elseif self.name == "MotorXEndSensorCheck" then
                                    ...
                                    elseif self.name == "SampleSensorCheck" then
                                        return dc:GetIMotorControl():GetSensorStatus(5)
                                    elseif self.name == "BlankSensorCheck" then
                                        return dc:GetIMotorControl():GetSensorStatus(6)
                                    elseif self.name == "ReagentSensorCheck" then
                                        return dc:GetIMotorControl():GetSensorStatus(7)
                                    elseif self.name == "SliderSensorCheck" then
                                        return dc:GetIMotorControl():GetSensorStatus(8)
                                    end
                                end)
    ...
    if not err then      -- 出现异常
        ...
        if ret == true then
            --if self.name == "SliderSensorCheck" or self.name == "SyringeInitSensorCheck" then
                self.result = "未遮挡"
            --else
            --    self.result = "遮挡"
            --end
