...
function MotorOperateFlow:OnProcess()
    ...
    elseif self.dst == setting.component.xPos.exposureCell then
        local runAction = Helper.Status.SetAction(setting.runAction.motorOperate.moveToExposureCell)
        StatusManager.Instance():SetAction(runAction)
    ...
    elseif self.dst == setting.component.xPos.wasteCell then
        ...
        StatusManager.Instance():SetAction(runAction)
    elseif self.dst == setting.component.xPos.stoveCell then
        local runAction = Helper.Status.SetAction(setting.runAction.motorOperate.moveToStoveCell)
        StatusManager.Instance():SetAction(runAction)
    end
    ...
    (
        function()
            if self.dst == setting.component.xPos.zero and self.step == 0 then
                ...
                -- if self.dst ~= setting.component.xPos.stoveCell and self.step > setting.motor.maxCupStepsZ then
                --     log:debug("超出运动范围")
                --     return false
                -- end
                -- op:AutoMove(self.dst, self.step)
                if self.dst == setting.component.xPos.stoveCell then
                    print("Singel Pos")
                    local mode = MotorMoveMode.Absolute 
                    if config.measureParam.measureType == 1 then
                        mode = MotorMoveMode.Relative
                    elseif config.measureParam.measureType == 2 then
                        mode = 3
                    end
                    Operator:MotorXMove(self.step, mode)
                else
                    print("Multiple Pos")    
                    local mode = MotorMoveMode.Absolute 
                    if config.measureParam.measureType == 1 then
                    ...
                    Operator:MotorXMoveXYZ(self.step, mode)
                end
