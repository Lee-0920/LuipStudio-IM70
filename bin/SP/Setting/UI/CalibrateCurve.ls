setting.ui.calibrateCurve =
{
    ...
    defaultRestore = function(profileTableName, path, curveParamName)
        ...
        for i = 1,setting.measure.range.rangeNum do
            ...
            config.measureParam.curveParam[i].RangeConsistencyIC = defaultTable.curveParam[i].RangeConsistencyIC
            config.measureParam.curveParam[i].ZeroConsistencyIC = defaultTable.curveParam[i].ZeroConsistencyIC
            config.measureParam.curveParam[i].timeStrIC = defaultTable.curveParam[i].timeStrIC
            config.measureParam.curveParam[i].curveBIC = defaultTable.curveParam[i].curveBIC
            config.measureParam.curveParam[i].curveKIC = defaultTable.curveParam[i].curveKIC
            config.measureParam.curveParam[i].RangeConsistencyNPOC = defaultTable.curveParam[i].RangeConsistencyIC
            config.measureParam.curveParam[i].ZeroConsistencyNPOC = defaultTable.curveParam[i].ZeroConsistencyIC
            config.measureParam.curveParam[i].timeStrNPOC = defaultTable.curveParam[i].timeStrIC
            config.measureParam.curveParam[i].curveBNPOC = defaultTable.curveParam[i].curveBIC
            config.measureParam.curveParam[i].curveKNPOC = defaultTable.curveParam[i].curveKIC
        end
    ...
    changeCurve = function(rangeValve, calibrateTime, currentCalibrate)
        ...
        for i = 1, rangeNum do
            if math.abs(config.system.rangeViewMap[i].view - rangeValve) < 0.000001 then
                if config.measureParam.measureType == ModelType.IC then
                    config.measureParam.curveParam[i].curveKIC = K
                    config.measureParam.curveParam[i].curveBIC = B
                    config.measureParam.curveParam[i].timeStrIC = calibrateTime
                    status.measure.calibrate[i].curveBIC = B
                    status.measure.calibrate[i].curveKIC = K
                elseif config.measureParam.measureType == ModelType.NPOC or config.measureParam.measureType == ModelType.CODN then
                    config.measureParam.curveParam[i].curveKNPOC = K
                    config.measureParam.curveParam[i].curveBNPOC = B
                    config.measureParam.curveParam[i].timeStrNPOC = calibrateTime
                    status.measure.calibrate[i].curveBNPOC = B
                    status.measure.calibrate[i].curveKNPOC = K
                else
    ...
    getExtraInfo = function(profileTableName)
        ...
        if config.measureParam.measureType == ModelType.IC then
            calibrateOne = string.format("%.1f", config.measureParam.curveParam[config.measureParam.range[config.measureParam.currentRange + 1]+1].ZeroConsistencyIC)
            calibrateTwo = string.format("%.1f", config.measureParam.curveParam[config.measureParam.range[config.measureParam.currentRange + 1]+1].RangeConsistencyIC)
        elseif config.measureParam.measureType == ModelType.NPOC
                or config.measureParam.measureType == ModelType.CODN then
            calibrateOne = string.format("%.1f", config.measureParam.curveParam[config.measureParam.range[config.measureParam.currentRange + 1]+1].ZeroConsistencyNPOC)
            calibrateTwo = string.format("%.1f", config.measureParam.curveParam[config.measureParam.range[config.measureParam.currentRange + 1]+1].RangeConsistencyNPOC)
        end
    ...
    getCurrentCurve = function(valveName)
        ...
        if valveName == 0 then
            ...
            if config.measureParam.measureType == ModelType.IC then
                result = config.measureParam.curveParam[config.measureParam.range[config.measureParam.currentRange + 1]+1].curveKIC
            elseif config.measureParam.measureType == ModelType.NPOC then
                result = config.measureParam.curveParam[config.measureParam.range[config.measureParam.currentRange + 1]+1].curveKNPOC
            end
        elseif valveName == 1 then
            ...
            if config.measureParam.measureType == ModelType.IC then
                result = config.measureParam.curveParam[config.measureParam.range[config.measureParam.currentRange + 1]+1].curveBIC
            elseif config.measureParam.measureType == ModelType.NPOC then
                result = config.measureParam.curveParam[config.measureParam.range[config.measureParam.currentRange + 1]+1].curveBNPOC
            end
    ...
    toInput = function(recordData, dateTime, curveK, curveB)
        ...
        recordData:PushByte(config.measureParam.measureType)
        return recordData
    end,
    ...
    curveExist = function()
        local model = config.measureParam.measureType
        local str = ""
        local ret = true
