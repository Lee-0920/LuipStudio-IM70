...
setting.liquid = --液路相关参数
{
    ...
    syringeMaxVolume = 2.5,            --注射器最大抽液体积
    syringeCleanVolume = 2.5,          --注射器更新体积
    syringeResetVolume = 0,          --注射器排空体积
    syringePipeVloume = 2.5,           --注射器管道体积
    syringeAddMaxVloume = 1,     --注射器加样限制最大体积
    syringeAddMinVloume = 0.01,     --注射器加样限制最小体积
    syringeUndoVolume = 0.05,        --防挂液排液体积
    syringeRelifeVolume = 0.03,      --催化剂再生排液体积
    syringeUpdateUpperLimit = 12,     --注射器最大更新次数
    syringeSuckAcceleration = 0.1,     --注射器抽液加速度
    syringeBaseVolume = 0.05,        --基准体积
    cleanAftMeaBlankVol = setting.unitVolume * 4,   --测量后清洗体积
    addLiquidSpeed = 0.8,
    addReagentSpeed = 0.08,
    addDiluteSampleSpeed = 0.5,
}
...
setting.temperature  =	--温度相关
{
    ...
    digestTemperatureLimitTop = 1250,    -- 消解温度输入上限,单位为℃
    reactTemperatureLimitTop = 1300,      -- 反应温度输入上限,单位为℃
    checkTemperatureTolerance = 1300,     --温度检查容忍值
    indexStove = 0,                 --燃烧炉
    indexRefrigerator = 1,          --制冷器
    indexLNDIR = 2,                  --红外检测器
    indexSNDIR = 3,                  --红外检测器
    temperatureStove = 0,    --燃烧炉温度
    temperatureRefrigerator = 1,    --制冷温度
    temperatureLNDIR = 2,           --测量池1温度
    temperatureSNDIR = 3,           --测量池2温度
    temperatureBoxLow = 4,          --下机箱温度
    temperatureInsideBox = 5,          --机箱温度
    boxDownFan = 0,                 --机箱风扇
    boxUpFan = 1,                   --预留风扇(电炉)
    indexStoveFan = 2,                  --电炉风扇
...
setting.measureResult =
{
    ...
    deviation = 0.0009,              --基线稳定判断范围 单位：V
    ...
    {
        {
            curveKUpLimit = 2,               -- K值上限
            curveKLowLimit = 0.65,	            -- K值下限
            curveBUpLimit = 1,                 -- 截距上限
            curveBLowLimit = 0,	               -- 截距下限
        },
        {
            curveKUpLimit = 1,               -- K值上限
            curveKLowLimit = 0.38,	            -- K值下限
            curveBUpLimit = 1,                 -- 截距上限
            curveBLowLimit = 0,	               -- 截距下限
        },
        {
            curveKUpLimit = 0.51,               -- K值上限
            curveKLowLimit = 0.12,	            -- K值下限
            curveBUpLimit = 1,                 -- 截距上限
            curveBLowLimit = 0,	               -- 截距下限
        },
        {
            curveKUpLimit = 0.105,               -- K值上限
            curveKLowLimit = 0.04,	            -- K值下限
            curveBUpLimit = 0,                 -- 截距上限
            curveBLowLimit = 0,	               -- 截距下限
        },
        {
            curveKUpLimit = 0.0149,               -- K值上限
            curveKLowLimit = 0.006,	            -- K值下限
            curveBUpLimit = 1,                 -- 截距上限
            curveBLowLimit = 0,	               -- 截距下限
        },
    },
...
setting.common =
{
    ...
    dataCopyDir = "/mnt/udisk/IM70-TOC_",     -- 数据复制路径
...
setting.measure.range  = --量程
{
    ...
    {--量程2
        ...
        rangeWindowMax = 550,	        -- 量程切换窗口上限
        sampleVolume = 0.5,			-- 水样体积，单位mL
        standardVolume = 0,			-- 量程校准液体积，单位mL
        zeroCheckVolume = 0,            -- 零点核查样体积，单位mL
        rangeCheckVolume = 0,            -- 量程核查样体积，单位mL
        blankVolume = 2,			-- 零点校准液体积，单位mL
        dilutionExtractVolume1 = 0,	-- 第1 次定量稀释抽取混合液体积，单位mL
        dilutionAddBlankVolume1 = 0,-- 第1 次定量稀释加零点校准液体积，单位mL
        dilutionExtractVolume2 = 0,	-- 第2 次定量稀释抽取混合液体积，单位mL
        dilutionAddBlankVolume2 = 0,-- 第2 次定量稀释加零点校准液体积，单位mL
    ...
    {--量程3
        ...
        sampleVolume = 0.2,			-- 水样体积，单位mL
        standardVolume = 0,			-- 量程校准液体积，单位mL
        zeroCheckVolume = 0,            -- 零点核查样体积，单位mL
        rangeCheckVolume = 0,            -- 量程核查样体积，单位mL
        blankVolume = 3.8,			-- 零点校准液体积，单位mL
        dilutionExtractVolume1 = 0,	-- 第1 次定量稀释抽取混合液体积，单位mL
        dilutionAddBlankVolume1 = 0,-- 第1 次定量稀释加零点校准液体积，单位mL
        dilutionExtractVolume2 = 0,	-- 第2 次定量稀释抽取混合液体积，单位mL
        dilutionAddBlankVolume2 = 0,-- 第2 次定量稀释加零点校准液体积，单位mL
    ...
    {--量程4
        ...
        sampleVolume = 0.1,			-- 水样体积，单位mL
        standardVolume = 0,			-- 量程校准液体积，单位mL
        zeroCheckVolume = 0,            -- 零点核查样体积，单位mL
        rangeCheckVolume = 0,            -- 量程核查样体积，单位mL
        blankVolume = 4.9,			-- 零点校准液体积，单位mL
        dilutionExtractVolume1 = 0,	-- 第1 次定量稀释抽取混合液体积，单位mL
        dilutionAddBlankVolume1 = 0,-- 第1 次定量稀释加零点校准液体积，单位mL
        dilutionExtractVolume2 = 0,	-- 第2 次定量稀释抽取混合液体积，单位mL
        dilutionAddBlankVolume2 = 0,-- 第2 次定量稀释加零点校准液体积，单位mL
        ...
        rinseRangeCheckVolume = 0,        -- 润洗量程核查样体积，单位mL
        rinseBlankVolume = 0,		-- 润洗零点校准液体积，单位mL
        diluteFactor = 50,           --稀释系数
        excursion = 1,
    },
    {--量程5
        viewRange = 50000,			            -- 用于显示的量程值
        rangeWindowMin = 9000,		-- 量程切换窗口下限
        rangeWindowMax = 55000,	        -- 量程切换窗口上限
        sampleVolume = 0.02,			-- 水样体积，单位mL
        standardVolume = 0,			-- 量程校准液体积，单位mL
        zeroCheckVolume = 0,            -- 零点核查样体积，单位mL
        rangeCheckVolume = 0,            -- 量程核查样体积，单位mL
        blankVolume = 3.8,			-- 零点校准液体积，单位mL
        dilutionExtractVolume1 = 0.2,	-- 第1 次定量稀释抽取混合液体积，单位mL
        dilutionAddBlankVolume1 = 0,-- 第1 次定量稀释加零点校准液体积，单位mL
        dilutionExtractVolume2 = 0,	-- 第2 次定量稀释抽取混合液体积，单位mL
        dilutionAddBlankVolume2 = 0,-- 第2 次定量稀释加零点校准液体积，单位mL
        dilutionExtractVolume3 =  0,	-- 第3 次定量稀释抽取混合液体积，单位mL
...
setting.sensor =	--传感器索引
{
    ...
    slidersForward = 0,
    slidersBackward = 1,
    slidersStep = 4000,     --滑块前进或后退固定步数，大于20时，滑块根据指定方向进行运动
    slidersStepOnly = 8000,     --仅方向移动设置步数，驱动版执行该参数时将默认除以2
...
setting.pressure =	--信号相关
{
    ...
    masterPressureMinLimit = 40,    --总阀压力下限告警值
...
setting.motor =   --电机相关
{
    ...
    limitStepsX = 7200,         --X轴限制步数
    limitStepsZ = 5000,         --Z轴限制步数
    maxStepsX = 7200,         --X轴限制步数
    maxStepsZ = 6500,         --Z轴限制步数
    allowResetStepsZ = 500,         --Z轴限制步数
    maxCupStepsZ = 4700,         --Z轴限制步数
    minStepsX = 0,         --X轴限制步数
    minStepsZ = 0,         --Z轴限制步数
