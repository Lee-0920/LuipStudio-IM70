        ...
        QWidget(parent)
{
    ...
    QStringList num;
    num<<tr("基本信号")<< tr("峰形图")<< tr("趋势图1")<<tr("趋势图2");
    ...
    //*********
    exPeakShapeWidget = new ExPeakShape();
    stackedWidget->addWidget(exPeakShapeWidget);
     //*********
...
void SingalWindow::itemClicked(QModelIndex index)
{
    ...
    strList.append(tr("趋势图1"));
    strList.append(tr("趋势图2"));
    ...
    {
        stackedWidget->setCurrentWidget(exPeakShapeWidget);
    }
}
...
SingalWindow::~SingalWindow()
{
    ...
    if (singlePeakShapeWidget)
        delete singlePeakShapeWidget;
    if (exPeakShapeWidget)
        delete exPeakShapeWidget;
