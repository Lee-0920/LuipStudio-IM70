        ...
        QWidget(parent)
{
    ...
    motorManngerTableWidget->setFixedSize(415,400);
    ...
    motorManngerTableWidget->horizontalHeader()->setStyleSheet("QHeaderView::section{background:rgb(210,210,210);}");
...
//    typeComboBox->setFixedSize(100,40);
    ...
    motorManngerTableWidget->setColumnWidth(0,240);    //设置每行或每列的高度或宽度
    if (m_tableInfo.userRow > 10)
    {
        motorManngerTableWidget->setColumnWidth(1,128);
    }
    else
    {
        motorManngerTableWidget->setColumnWidth(1,158);
    }

    for(int i = 0;i < motorManngerTableWidget->rowCount();i++)
    {
        motorManngerTableWidget->setRowHeight(i, 60);
    }
