        ...
        QWidget(parent)
{
    ...
    m_strList<<tr("维护")<<tr("组合维护")<<tr("管道操作")<<tr("硬件测试")<<tr("电机操作")<<tr("状态查询")<<tr("试剂管理")<<tr("硬件校准")<<tr("组合操作")<<tr("升级")
    ...
    motorManager = new MotorManagerWidget();
    sensorCheck = new SensorCheckWidget();
    ...
    stackedWidget->addWidget(motorManager);
    stackedWidget->addWidget(sensorCheck);
    ...
    luaEngine->GetLuaValue(state, "setting.ui.operation.motorOperator.readPrivilege", privilege);
    m_privilegeList << privilege;

    luaEngine->GetLuaValue(state, "setting.ui.diagnosis.sensorCheck.readPrivilege", privilege);
    m_privilegeList << privilege;
...
void RemoteMainTainWindow::itemClicked(QModelIndex index)
{
    ...
    {
        stackedWidget->setCurrentWidget(motorManager);
    }
    else if(m_strList.at(5) == index.data().toString())
    {
        stackedWidget->setCurrentWidget(sensorCheck);
    }
    ...
    else if(m_strList.at(10) == index.data().toString())
    {
        stackedWidget->setCurrentWidget(userResource);
    }
    else if (m_strList.at(11) == index.data().toString())
    {
