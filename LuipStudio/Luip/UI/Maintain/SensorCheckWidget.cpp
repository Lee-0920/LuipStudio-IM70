        ...
        QWidget(parent)
{
    ...
    sensorCheckTableWidget->setFixedSize(580, 340);
    ...
    sensorCheckTableWidget->horizontalHeader()->setFixedHeight(45); // 设置表头的高度
    if (m_tableInfo.userRow > m_tableInfo.perPageRow)
    {
        sensorCheckTableWidget->verticalScrollBar()->setStyleSheet
                ("QScrollBar{background:transparent; width: 35px;}");
    ...
    sensorCheckTableWidget->setColumnWidth(0,110);    //设置每行或每列的高度或宽度
    sensorCheckTableWidget->setColumnWidth(1,60);
    sensorCheckTableWidget->setColumnWidth(2,210);
    if (m_tableInfo.userRow > m_tableInfo.perPageRow)
    {
        sensorCheckTableWidget->setColumnWidth(3, 160);
    }
    else
    {
        sensorCheckTableWidget->setColumnWidth(3, 200);
    }

    for(int i = 0;i < sensorCheckTableWidget->rowCount();i++)
    {
        sensorCheckTableWidget->setRowHeight(i,40);
    }
