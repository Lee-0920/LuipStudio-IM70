...
#define RESULT_WIDGET_MIX_ROW_COUNT     2
#define RESULT_WIDGET_MIX_COLUMN_COUNT  3
#define RESULT_WIDGET_MIX_WIDGET        220
#define RESULT_WIDGET_MIX_HEIGHT        180
...
void RunStatusWindow::initLeftTop()
{
    ...
    oolua_ipairs(targetTable)
    {
        ...
        if (isTarget == false || (type & targetPrivilege))
        {
            ...
            iterm.isNeedModelType = itermTable.safe_at("getModelType", iterm.getModelType);
            m_items.push_back(iterm);

            targetList.append(QString(iterm.name.c_str()));

            if(m_resultItermList.size() + 1 >
                    RESULT_WIDGET_MIX_COLUMN_COUNT * RESULT_WIDGET_MIX_ROW_COUNT)
            {
                qDebug("Show Limited");
                break;
            }

            QString name = iterm.name.c_str();
            QString target = "TC";
            QString unitStr = "mg/L";
            ResultWidget* pWidget = new ResultWidget(name,target, tr("0.0000"),
                                                     tr("0000-00-00 00:00:00"), unitStr, this);
            pWidget->setFixedSize(RESULT_WIDGET_MIX_WIDGET, RESULT_WIDGET_MIX_HEIGHT);
            m_resultItermList.append(pWidget);
        }
    }
    oolua_ipairs_end()
...
////    m_resultPanel->setFixedSize(800, 400);
     ...
     m_pLayoutG = new QGridLayout();
     int row = 0;
     int column = 0;
     int nCountAdded = 0;
     for (QList<ResultWidget*>::const_iterator it = m_resultItermList.begin();
          it != m_resultItermList.end(); it++)
     {
         if (column >= RESULT_WIDGET_MIX_COLUMN_COUNT)
         {
             row++;
             column = 0;
         }

         if (row >= RESULT_WIDGET_MIX_ROW_COUNT)
         {
             //超过 RESULT_WIDGET_MIX_ROW_COUNT * RESULT_WIDGET_MIX_COLUMN_COUNT 不显示
             m_resultItermList.removeLast();
             break;
         }

         // 第row行，第column列开始，占1行1列
         m_pLayoutG->addWidget(*it, row, column, 1, 1, Qt::AlignCenter);
     ...
     //填补空白
     for (; nCountAdded < RESULT_WIDGET_MIX_COLUMN_COUNT * RESULT_WIDGET_MIX_ROW_COUNT; ++nCountAdded)
     {
         if (column >= RESULT_WIDGET_MIX_COLUMN_COUNT)
         {
             row++;
         ...
         // 第row行，第column列开始，占1行1列
         m_pLayoutG->addItem(new QSpacerItem(RESULT_WIDGET_MIX_WIDGET, RESULT_WIDGET_MIX_HEIGHT), row, column, 1, 1, Qt::AlignCenter);

         column++;
     }

     // 设置水平间距
     m_pLayoutG->setHorizontalSpacing(0);
     // 设置垂直间距
     m_pLayoutG->setVerticalSpacing(40);
     // 设置外间距
     m_pLayoutG->setContentsMargins(0, 0, 0, 0);


//    QHBoxLayout * measureResultPanelLayout = new QHBoxLayout();
//    measureResultPanelLayout->addWidget(m_pLayoutG);
...
//    rangeCheckButton->setGraphicsEffect(rangeCheckEffect);
    ...
    fatalButton->setFixedSize(230,50);
...
//    buttonVLayout->setContentsMargins(0, 0, 0, 0);
    ...
    estimateResultTypeFont.setPointSize(15);
    ...
    erLayout->addStretch();
    erLayout->addLayout(m_pLayoutG);
     erLayout->setSpacing(10);
    ...
    try
    {
        if (!m_items.empty())
        {
            ...
            for(int i = 0; i < (int)m_items.size(); ++i)
            {
                ...
                if (iterm.isNeedModelType)
                {
                    Byte modelType;
                    call(iterm.getModelType);
                    OOLUA::pull(newState, modelType);
...
//                    if (modelType >= 0)
                    {
                        ...
                        optionTable.safe_at((int)modelType + 1, strModelType);
                    }
                }

                if(i < m_resultItermList.size())
                {
                    ResultWidget* pWidget = m_resultItermList.at(i);
                    pWidget->SetResultInfo(QObject::tr(iterm.name.c_str()), QString(strModelType.c_str()), strConsistency, strTime, QString(strResultType.c_str()));
                    pWidget->SetMeasureResultFont(0);
                }
...
void RunStatusWindow::SlotFatalButton()
{
