...
{   
    ...
    listView->verticalScrollBar()->setStyleSheet("QScrollBar{background:transparent; width: 40px;}");


    stackedWidget = new QStackedWidget();
    stackedWidget->setFixedHeight(425);
    stackedWidget->setFixedWidth(693);
    ...
    if (isExistMeasureData)
    {
        oolua_ipairs(measureDataTable)
        {
            ...
            if (isTarget == false || (type & targetPrivilege))
            {
                ...
                m_strList << QObject::tr(name.c_str());
                ...
                if(m_strList.size() > 1)
                {
                    measureDataWidget->HideTableColumn(4);
                }
            }
        }
        oolua_ipairs_end()
    ...
    if (isExistWaveData)
    {
        ...
        oolua_ipairs(waveDataTable)
        {
            ...
            if (isTarget == false || (type & targetPrivilege))
            {
                ...
                itermTable.at("name", name);
//                nameList[waveMember++] = name;
                nameList<< QObject::tr(name.c_str());
//                m_strList << QObject::tr(name.c_str());

                itermTable.at("resultDataname", resultDataname);
                resultDatanameList[waveMember++] = resultDataname;

                itermTable.at("profileTableName", profileTableName);

                itermTable.at("rangeTableName", rangeTableName);

//                waveDataWidget = new WaveDataManager(resultDataname, profileTableName, rangeTableName);
...
//                m_waveDataWidget.push_back(waveDataWidget);
        ...
        if(waveMember > 0)
        {
             m_strList << QObject::tr("历史趋势");
             waveDataManager = new WaveDataManager(nameList, resultDatanameList, profileTableName, rangeTableName);
             stackedWidget->addWidget(waveDataManager);
        }
    ...
    if (isExistCalibrateCurve)
    {
        oolua_ipairs(calibrateCurveTable)
        {
            ...
            if (isTarget == false || (type & targetPrivilege))
            {
                ...
                calibrateCurveWidget->HideColumn(4);
            }
        }
        oolua_ipairs_end()
    ...
    {
        curveIndex = m_strList.size();
    }
//    m_strList <<tr("量程信息");
//    curveWidget = new CurveTableWidget();
//    stackedWidget->addWidget(curveWidget);
    ...
    stackedWidgetLayout->setContentsMargins(0, 0, 0, 0);
    ...
    qDebug("MeasureData List %d", m_strList.size());
    if(m_strList.size() > 7)
    {
        listView->setFixedWidth(151);
        stackedWidget->setFixedWidth(643);
        for (int i = 0; i <= m_measureDataEndIndex; i++)
        {
            MeasureData *measureDataWidget = m_measureDataWidget.at(i);
            measureDataWidget->Resize();
        }
//        for (int i = 0; i <= m_calibrateCurveEndIndex; i++)
//        {
            CalibrateCurve *calibrateCurveWidget = m_calibrateCurveWidget.at(0);
            calibrateCurveWidget->Resize();
//        }
        waveDataManager->Resize();
//        curveWidget->Resize();
        warnManagerWidget->Resize();
        runLogWidget->Resize();
    }
    ...
    mainlayout->setContentsMargins(2, 5, 10, 0);
...
{
//    if(m_strList.at(curveIndex) == index.data().toString())
//    {
//        stackedWidget->setCurrentWidget(curveWidget);
//    }
    ...
    else if (m_strList.at(m_waveDataEndIndex) == index.data().toString())//(m_waveDataWidget.size() != 0)
    {
        stackedWidget->setCurrentWidget(waveDataManager);
    }
    else
    {
        int startIndex = 0;
