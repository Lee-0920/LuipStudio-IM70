        ...
        m_profileTableName(profileTableName),m_profileFileName(profileFileName),m_curveParamName(curveParamName),m_isDisplayHiddenInfo(false), m_isXYChangeing(false)
{
    ...
    m_isCurveExist = fieldTable.safe_at("curveExist", m_isCurveExistFunc);
...
void CalibrateCurve::ToMap()
{
    ...
    if (m_isCurveExist)
    {
        m_lua->call(m_isCurveExistFunc);
        m_lua->pull(valid);
        m_lua->pull(str);
        if(valid == false)
        {
            QString infoText("请选择正确的测量参数！\n");
            infoText = infoText + QString("当前测量参数：") + QString(str.data());
    ...
    try
    {
        ...
        if(j!=-1)
        {
            ...
            QString currentType = calibrateTableWidget->item(j ,5)->text();
            ...
            if (!currentCalibrate.isEmpty())
            {
                ...
                if(m_isCurveExist && (QString::compare(QString(str.data()), currentType)))
                {
                    QString infoText("测量参数不一致！");
                    infoText = infoText + QString("\n标线测量参数：")+  currentType + QString("\n当前测量参数：") + QString(str.data());
                    MessageDialog msg(infoText, this, MsgStyle::ONLYOK);
                    if(msg.exec()== QDialog::Accepted)
                    {
                        return ;
...
void CalibrateCurve::Resize(void)
{
//     calibrateTableWidget->resize(549,344);
     calibrateTableWidget->setFixedSize(544,321);
     this->TableExtraInfoAdjust(true);
     minTime->setFixedWidth(105);
     maxTime->setFixedWidth(105);
     searchButton->setFixedSize(70, 40);
     toMapButton->setFixedSize(90, 40);
     inputButton->setFixedSize(70, 40);
...
void CalibrateCurve::HideColumn(int index)
{
    calibrateTableWidget->horizontalScrollBar()->setVisible(false);
    calibrateTableWidget->horizontalScrollBar()->setDisabled(true);
    if(index < calibrateTableWidget->columnCount())
    {
        calibrateTableWidget->hideColumn(index);
    }
}
