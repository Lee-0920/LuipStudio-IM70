...
int SelfSortData(QPointF data1, QPointF data2);

namespace UI
{

WaveDataManager::WaveDataManager(QStringList name, System::String *resultDataname, System::String profileTableName, System::String rangeTableName, QWidget *parent)
    :QWidget(parent),m_consistency("consistency"), m_datetime("dateTime"), m_resultType("resultType"),m_modelType("modelType"),m_curMaxConsistency(0),m_curMinConsistency(0),
      m_profileTableName(profileTableName), m_rangeTableName(rangeTableName),m_resultDataname(resultDataname[0]),m_name(name)
{
    MaintainUserChange::Instance()->Register(this);
    UpdateWidgetManager::Instance()->Register(this);
    ResultManager::Instance()->Register(this);

    m_resultDatanameList[0] = resultDataname[0];
    m_resultDatanameList[1] = resultDataname[1];
    m_resultDatanameList[2] = resultDataname[2];
    m_resultDatanameList[3] = resultDataname[3];
    m_resultDatanameList[4] = resultDataname[4];
    m_resultDatanameList[5] = resultDataname[5];
    m_resultDatanameList[6] = resultDataname[6];
...
void WaveDataManager::Resize(void)
{
//     WaveDataTableWidget->setFixedSize(530,361);
     resize(650, 400);
     plot->setFixedSize(630, 385);
}

WaveDataManager::~WaveDataManager()
{

}

void WaveDataManager::InitCanvas()
{
    ...
    nameLabel->setFixedSize(80,36);
    nameLabel->setText(tr("通道选择"));
    nameLabel->setFont(nextFont);
    ...
    nameCombox->setFixedSize(150,36);
    nameCombox->setFont(nextFont);
    nameCombox->insertItems(0, m_name);

    QHBoxLayout *nameComboxLayout = new QHBoxLayout();
    nameComboxLayout->addWidget(nameCombox);
    nameComboxLayout->setSpacing(0);
    nameComboxLayout->setContentsMargins(15, 0, 0, 0);

    QHBoxLayout *bottomLayout = new QHBoxLayout();
    bottomLayout->addStretch();
    bottomLayout->addLayout(nameLabelLayout);
    bottomLayout->addLayout(nameComboxLayout);
    ...
    connect(nameCombox, SIGNAL(currentIndexChanged(int)), this, SLOT(SlotNameChanged(int)));
    ...
    for (map<QString, WaveDataManagerCategory>::iterator iter = m_waveDataCategory.begin(); iter != m_waveDataCategory.end(); ++iter)
    {
        WaveDataManagerCategory &category = iter->second;
        std::sort(category.buffer.begin(), category.buffer.end(), SelfSortData);
        category.curve->attach(plot);
        category.curve->setSamples(category.buffer);
    }
}

void WaveDataManager::SetDisplayDays()
{
    ...
    oolua_ipairs(itermsTable)
    {
        ...
        WaveDataManagerCategory category;
...
void WaveDataManager::OnMeasureResultAdded(String name, RecordData result)
{
    (void)name;
    (void)result;
...
void WaveDataManager::OnCalibrateResultAdded(String name, RecordData result)
{
    (void)name;
    (void)result;
}

void WaveDataManager::UpdaterData()
{
...
int WaveDataManager::DateTimeToInt(QDateTime dateTime)
{
...
void WaveDataManager::CurrentWaveChange()
{
    this->SetMaxAxisScale(m_curMinConsistency,m_curMaxConsistency);
    plot->autoRefresh();
...
void WaveDataManager::ReadMeasureResultLogFile()
{
    QString resultType;
    QString modelType;
    float consistency;
    ...
    while(m_resultFiles->HasPrevious(m_resultFiles->GetSelfReaderIndex()) && i < 5000)
    {
        ...
        if(theDate >= m_ObjBackDateTime && theDate <= m_ObjNextDateTime)
        {
...
void WaveDataManager::AddPoint(QDateTime dateTime, double value, QVector<QPointF> &vector)
{
...
void WaveDataManager::showEvent(QShowEvent *event)
{
    (void)event;
...
void WaveDataManager::OnUserChange()
{
}

void WaveDataManager::NextItem()
{
    m_ObjBackDateTime = m_ObjBackDateTime.addDays(7);
    m_ObjNextDateTime = m_ObjNextDateTime.addDays(7);
...
QDateTime WaveDataManager::GetBackDateTime(int iBackDays)
{
    // 设置开始的时间
    QDateTime objDateTime = QDateTime::currentDateTime();
...
void WaveDataManager::OnUpdateWidget(UpdateEvent event, System::String message)
{
...
void WaveDataManager::SetMaxAxisScale(float minConsistency,float maxConsistency)
{
...
void WaveDataManager::SlotNameChanged(int index)
{
//    qDebug("change %d", index);
    m_resultDataname = m_resultDatanameList[index];
//    qDebug() << m_resultDataname.c_str();
    m_resultFiles = ResultManager::Instance()->GetWaveUseMeasureRecordFile(m_resultDataname);
//    Resize();
    SetDisplayDays();
    for (map<QString, WaveDataManagerCategory>::iterator iter = m_waveDataCategory.begin(); iter != m_waveDataCategory.end(); ++iter)
