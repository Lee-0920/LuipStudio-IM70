 ...
 * @param[in] addr 设备地址。
 */
ExtSolenoidValveInterface::ExtSolenoidValveInterface(DscpAddress addr)
 ...
 */
int ExtSolenoidValveInterface::GetTotalValves()
{
    int totalValves = 0;
 ...
 */
ExValveMap ExtSolenoidValveInterface::GetValveMap()
{
    ExValveMap map(0);
...
* @return 返回状态，操作是否成功。
 */
bool ExtSolenoidValveInterface::SetValveMap(ExValveMap map)
{
...
* @return 返回状态，操作是否成功。
 */
bool ExtSolenoidValveInterface::SetValveMapNormalOpen(ExValveMap map)
{
    Uint16 status = DscpStatus::Error;
    Uint32 mapVal = map.GetData();
