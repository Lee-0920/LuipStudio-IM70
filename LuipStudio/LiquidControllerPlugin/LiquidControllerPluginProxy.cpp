 ...
 * @details
 */
OOLUA_PROXY(,ExMotionParam)
    OOLUA_MGET_MSET(acceleration, GetAcceleration, SetAcceleration)
    OOLUA_MGET_MSET(speed, GetSpeed, SetSpeed)
OOLUA_PROXY_END
OOLUA_EXPORT_FUNCTIONS(ExMotionParam, SetAcceleration, SetSpeed)
OOLUA_EXPORT_FUNCTIONS_CONST(ExMotionParam, GetAcceleration, GetSpeed)

/**
 * @brief 蠕动泵状态。
 * @details
 */
OOLUA_PROXY(,ExPumpStatus)
    OOLUA_TAGS(
...
OOLUA_PROXY_END
OOLUA_EXPORT_NO_FUNCTIONS(ExPumpStatus)  // 导出函数声明（没有函数也需要声明）
 ...
 * @details
 */
OOLUA_PROXY(,ExPumpOperateResult)
    OOLUA_TAGS(
        Register_class_enums  // 枚举需要特别标注，否则枚举值无效
    )
    OOLUA_ENUMS(
...
OOLUA_PROXY_END
OOLUA_EXPORT_NO_FUNCTIONS(ExPumpOperateResult)  // 导出函数声明（没有函数也需要声明）

/**
 * @brief 蠕动泵结果。
 * @details
 */
OOLUA_PROXY(,ExPumpResult)
    OOLUA_MGET_MSET(index, GetIndex, SetIndex)
    OOLUA_MGET_MSET(result, GetResult, SetResult)
OOLUA_PROXY_END
OOLUA_EXPORT_FUNCTIONS(ExPumpResult, SetIndex, SetResult)
OOLUA_EXPORT_FUNCTIONS_CONST(ExPumpResult, GetIndex, GetResult)

/**
 * @brief 阀映射图。
 * @details
 */
OOLUA_PROXY(,ExValveMap)
...
OOLUA_PROXY_END
OOLUA_EXPORT_FUNCTIONS(ExValveMap, SetData, GetData, SetOn, SetOff, IsOn, clear)
OOLUA_EXPORT_FUNCTIONS_CONST(ExValveMap)
...
/**
 * @brief 扩展蠕动泵控制接口。
 * @details 定义了一序列扩展蠕动泵控制相关的操作。
 */
OOLUA_PROXY(,ExtPeristalticPumpInterface, DeviceInterface)
    OOLUA_TAGS(
        No_default_constructor //没有默认构造函数时可以使用此标签
    )
    OOLUA_CTORS(
...
OOLUA_PROXY_END
OOLUA_EXPORT_FUNCTIONS(ExtPeristalticPumpInterface, GetTotalPumps, GetPumpFactor, SetPumpFactor,
                       GetMotionParam, SetMotionParam, GetPumpStatus, StartPump,
                       StopPump, GetPumpVolume, ExpectPumpResult)
OOLUA_EXPORT_FUNCTIONS_CONST(ExtPeristalticPumpInterface)

/**
 * @brief 扩展电磁阀控制接口。
 * @details 定义了一序列电磁阀控制相关的操作。
 */
OOLUA_PROXY(,ExtSolenoidValveInterface, DeviceInterface)
    OOLUA_TAGS(
        No_default_constructor //没有默认构造函数时可以使用此标签
    )
    OOLUA_CTORS(
...
OOLUA_PROXY_END
OOLUA_EXPORT_FUNCTIONS(ExtSolenoidValveInterface, GetTotalValves, GetValveMap, SetValveMap)
OOLUA_EXPORT_FUNCTIONS_CONST(ExtSolenoidValveInterface)
...
//    OOLUA_MEM_FUNC(void, Register, cpp_in_p<ISignalNotifiable*>)
    ...
    OOLUA_MFUNC(GetIExtPeristalticPump)
    OOLUA_MFUNC(GetIExtSolenoidValve)

OOLUA_PROXY_END
OOLUA_EXPORT_FUNCTIONS(LiquidController, Init, Uninit, Register,
                       GetIExtPeristalticPump, GetIExtSolenoidValve)
...
//    lua->register_class<MeterPoints>();
    lua->register_class<ExMotionParam>();
    lua->register_class<ExPumpStatus>();
    lua->register_class<RollDirection>();
    lua->register_class<ExPumpOperateResult>();
    lua->register_class<ExPumpResult>();
    lua->register_class<ExValveMap>();
...
//    lua->register_class<SolenoidValveInterface>();
    lua->register_class<ExtPeristalticPumpInterface>();
    lua->register_class<ExtSolenoidValveInterface>();
