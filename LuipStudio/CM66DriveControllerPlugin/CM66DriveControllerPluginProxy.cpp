 ...
 * @details
 */
OOLUA_PROXY(,PumpFixedType)
    OOLUA_TAGS(
        Register_class_enums  // 枚举需要特别标注，否则枚举值无效
    )
    OOLUA_ENUMS(
        OOLUA_ENUM(ClockwiseDrainAnticlockwiseSuck)   // 枚举值
        OOLUA_ENUM(ClockwiseSuckAnticlockwiseDrain)   // 枚举值
    )
OOLUA_PROXY_END
OOLUA_EXPORT_NO_FUNCTIONS(PumpFixedType)  // 导出函数声明（没有函数也需要声明）
...
OOLUA_PROXY(,MeterResult)
    ...
    OOLUA_ENUMS(
        ...
        OOLUA_ENUM(NotFindBoundary)   // 枚举值
    )
OOLUA_PROXY_END
...
OOLUA_MFUNC_CONST(GetMeterLedLevel)
OOLUA_MFUNC_CONST(SetMeterLedLevel)
OOLUA_MFUNC_CONST(StartStaticADControl)
OOLUA_MFUNC_CONST(StopStaticADControl)
OOLUA_MFUNC_CONST(GetStaticADControlParam)
OOLUA_MFUNC_CONST(SetStaticADControlParam)
...
OOLUA_EXPORT_FUNCTIONS(OpticalMeterInterface, TurnOnLED, TurnOffLED,
                       ...
                       SetMeteSpeed, SetMeterFinishValveMap, RegisterOpticalADNotice, ExpectStaticADControlResult)
OOLUA_EXPORT_FUNCTIONS_CONST(OpticalMeterInterface, GetPumpFactor, GetMeterPoints,
                             ...
                             GetMeterLedLevel, SetMeterLedLevel, StartStaticADControl, StopStaticADControl,
                             GetStaticADControlParam, SetStaticADControlParam, IsStaticADControlValid)
...
OOLUA_PROXY(,PeristalticPumpInterface, DeviceInterface)
    ...
    OOLUA_MFUNC(GetPumpFixedType)
    OOLUA_MFUNC(SetPumpFixedType)
...
OOLUA_EXPORT_FUNCTIONS(PeristalticPumpInterface, GetTotalPumps, GetPumpFactor, SetPumpFactor,
                       GetMotionParam, SetMotionParam, GetPumpStatus, StartPump, GetPumpFixedType,
                       SetPumpFixedType, StopPump, GetPumpVolume, ExpectPumpResult,GetSenseStatus)
...
OOLUA_PROXY(,TemperatureControlInterface, DeviceInterface)
    ...
    OOLUA_MFUNC_CONST(GetHeaterMaxDutyCycle)      ///<获取加热丝最大占空比
...
//    OOLUA_MEM_FUNC(void, RegisterTemperatureNotice, cpp_in_p<IEventReceivable*>)
    ...
    OOLUA_MFUNC(TemperatureCalibrate)
...
OOLUA_EXPORT_FUNCTIONS(TemperatureControlInterface,
                       ...
                       ExpectThermostat,
                       BoxFanSetOutput,
                       DigestionFanSetOutput,
                       RegisterTemperatureNotice,
                       SetThermostatParam,
                       SetHeaterMaxDutyCycle,
                       SetCurrentThermostatParam,
...
OOLUA_EXPORT_FUNCTIONS_CONST(TemperatureControlInterface,
                             ...
                             GetThermostatParam,
                             GetThermostatStatus,
                             GetHeaterMaxDutyCycle,
                             GetCurrentThermostatParam)

/**
 * @brief 滴定数据。
 * @details
 */
OOLUA_PROXY(,TitrationData)
    OOLUA_MGET_MSET(electrodeAD, GetElectrodeAD, SetElectrodeAD)
    OOLUA_MGET_MSET(titrationStep, GetTitrationStep, SetTitrationStep)
OOLUA_PROXY_END
OOLUA_EXPORT_FUNCTIONS(TitrationData, SetElectrodeAD, SetTitrationStep)
OOLUA_EXPORT_FUNCTIONS_CONST(TitrationData, GetElectrodeAD, GetTitrationStep)

/**
 * @brief 滴定信号采集接口。
 * @details 定义电极滴定相关的操作。
 */
OOLUA_PROXY(,TitrationOperateInterface, DeviceInterface)
    OOLUA_TAGS(
        No_default_constructor //没有默认构造函数时可以使用此标签
    )
    OOLUA_CTORS(
    ...
    OOLUA_MFUNC(SetAcquireElectrodeADNotifyPeriod)
    OOLUA_MFUNC(StartTitration)
    OOLUA_MFUNC(StopTitration)
    OOLUA_MFUNC(GetTitrationStatus)
    OOLUA_MFUNC(GetElectrodeSignal)
    OOLUA_MFUNC(SetTitrationSpeed)
    OOLUA_MFUNC(ExpectTitrationData)
    OOLUA_MFUNC(TitrationElectrodeOutput)
    OOLUA_MFUNC(RegisterAcquireElectrodeADNotice)

OOLUA_PROXY_END
OOLUA_EXPORT_FUNCTIONS(TitrationOperateInterface, SetAcquireElectrodeADNotifyPeriod, StartTitration, StopTitration,
                       GetTitrationStatus, GetElectrodeSignal, SetTitrationSpeed, ExpectTitrationData, TitrationElectrodeOutput, RegisterAcquireElectrodeADNotice)
OOLUA_EXPORT_FUNCTIONS_CONST(TitrationOperateInterface)

/**
 * @brief 驱动控制器。
 * @details
 */
OOLUA_PROXY(,CM66DriveController, BaseController)
    OOLUA_TAGS(
        No_default_constructor //没有默认构造函数时可以使用此标签
    )
    OOLUA_CTORS(
    ...
    OOLUA_MFUNC(GetITitrationOperate)
    ...
    OOLUA_MFUNC(ClearAllRemainEvent)
    OOLUA_MFUNC(ClearThermostatRemainEvent)
    OOLUA_MFUNC(ClearPumpRemainEvent)
    OOLUA_MFUNC(GetPacketTitrationData)

OOLUA_PROXY_END
OOLUA_EXPORT_FUNCTIONS(CM66DriveController, Init, Uninit, GetIPeristalticPump, GetISolenoidValve, GetIOpticalMeter,GetCurrentTemperature,
                       GetITemperatureControl, GetITitrationOperate, ClearAllRemainEvent, ClearThermostatRemainEvent, ClearPumpRemainEvent,GetPacketTitrationData
                       , Register)
OOLUA_EXPORT_FUNCTIONS_CONST(CM66DriveController,
                             GetDigestTemperature,
                             GetEnvironmentTemperature)

CM66DriveControllerPluginProxy::CM66DriveControllerPluginProxy()
{

}

void CM66DriveControllerPluginProxy::Proxy()
{
    ...
    lua->register_class<PumpFixedType>();
    ...
    lua->register_class<TitrationData>();
    lua->register_class<TitrationOperateInterface>();
    lua->register_class<CM66DriveController>();
}
