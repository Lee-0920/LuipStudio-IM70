 ...
 * @note 该命令将立即返回，电机移动完成将以事件的形式上报。
 */
bool MotorControlInterface::MotorMoveXYZ(Motor3DPos xyzPos, MotorMoveMode mode)
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[13];

    memcpy(cmdData, &mode, sizeof(MotorMoveMode));
    memcpy(cmdData+1, &xyzPos, sizeof(Motor3DPos));

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_MCI_MOTOR_MOVE_XYZ, cmdData, sizeof(cmdData)));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);
    return (status == DscpStatus::OK);
}
 ...
 *  - @ref DSCP_ERROR_PARAM 参数错误，传入的参数有问题
 */
bool MotorControlInterface::MotorResetXYZ(void)
{
    Uint16 status = DscpStatus::Error;

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_MCI_MOTOR_RESET_XYZ));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);
    return (status == DscpStatus::OK);
}
 ...
 * @return gstat Uint8，驱动全局寄存器的值
 */
bool MotorControlInterface::ExtraDeviceControl(Uint8 index, Uint8 value)
{
    Uint16 status = DscpStatus::Error;
    ...
    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_MCI_EXTRADEVICE_CONTROL, cmdData, sizeof(cmdData)));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);
    return (status == DscpStatus::OK);
}
