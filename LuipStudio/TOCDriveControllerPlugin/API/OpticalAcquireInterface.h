...
public:
    ...
    bool StartLEDOnceAdjust(float targetAD, float tolorance, Uint32 timeout);
    // 停止LED单次定向调节至目标值操作
...
//    bool SetLEDDefaultValue(float value);
    ...
    // 设置红外模块测量值上报周期
    bool SetNDIRReportPeriod(float value);

    // 注册信号AD定时上报事件。
    void RegisterNDIRNotice(IEventReceivable *handle);

    // 注册漏液检测AD定时上报事件。
    void RegisterAcquireCheckLeaking(IEventReceivable *handle);

    // 设置漏液信号AD上报周期。
    bool SetCheckLeakingADNotifyPeriod(float period);
};
