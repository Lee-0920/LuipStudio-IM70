 ...
 * @return 返回状态，操作是否成功。
 */
bool OpticalAcquireInterface:: TurnOnLED(Uint8 index) const
{
    Uint16 status = DscpStatus::Error;

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_OAI_TURN_ON_LED, &index, sizeof(index)));
 ...
 * @return 返回状态，操作是否成功。
 */
bool OpticalAcquireInterface:: TurnOffLED(Uint8 index) const
{
    Uint16 status = DscpStatus::Error;

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_OAI_STOP_LEDCONTROLLER, &index, sizeof(index)));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);
    return (status == DscpStatus::OK);
}
 ...
 *  - @ref DSCP_ERROR 操作失败；
 */
bool OpticalAcquireInterface::StartLEDOnceAdjust(float targetAD, float tolerance, Uint32 timeout)
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[12] = {0};
...
//    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_OAI_SET_LED_DEFAULT_VALUE, &value, sizeof(float)));
//    SyncCaller syncCaller(m_retries);
//    status = syncCaller.SendWithStatus(cmd);
//    return (status == DscpStatus::OK);
//}
 ...
 * @return Float32，电压V：
 */
float OpticalAcquireInterface::GetLEDDefaultValue(Uint8 index) const
{
    float value = 0;

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_OAI_GET_LED_DEFAULT_VALUE, &index, sizeof(index)));
    SyncCaller  syncCaller(m_retries);
    DscpRespPtr resp = syncCaller.Send(cmd);
    if (resp)
    {
 ...
 *  - @ref DSCP_ERROR 操作失败；
 */
bool OpticalAcquireInterface::SetLEDDefaultValue(Uint8 index, float value)
{
    Uint16 status = DscpStatus::Error;
    Uint8 data[5];
    ...
    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_OAI_SET_LED_DEFAULT_VALUE, data, sizeof(data)));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);
    return (status == DscpStatus::OK);
}
 ...
 *  - @ref DSCP_ERROR 操作失败；
 */
bool OpticalAcquireInterface::SetNDIRReportPeriod(float value)
{
    Uint16 status = DscpStatus::Error;

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_OAI_SET_REPORT_NOTIFY_PERIOD, &value, sizeof(value)));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);
    return (status == DscpStatus::OK);
}
...
/**
 * @brief 注册测量值定时上报事件。
 * @details 系统将根据设置的上报周期，定时向上汇报测量值。
 *  上报周期可通过命令 @ref DSCP_CMD_OAI_SET_SIGNAL_AD_NOTIFY_PERIOD 设定。
 * @param[in] handle 定时接收测量值上报事件的对象。
 */
void OpticalAcquireInterface::RegisterNDIRNotice(IEventReceivable *handle)
{
    EventHandler::Instance()->Register(m_addr,DSCP_EVENT_OAI_REPORT_DATA,handle);
    EventHandler::Instance()->Register(m_addr,DSCP_EVENT_OAI_REPORT_SMEA_DATA,handle);
}
...
bool OpticalAcquireInterface::SetCheckLeakingADNotifyPeriod(float period)
{
    Uint16 status = DscpStatus::Error;
