...
/**
 * @brief 设置DAC0-1通道输出值。
 * @param index 通道索引0-1，value DAC输出值0-3.25。
 * @return 状态回应，Uint16，支持的状态有：
 *  - @ref DSCP_OK  操作成功；
 *  - @ref DSCP_ERROR 操作失败；
 */
bool SolenoidValveInterface::ProportionalValve_SetDACValue(Uint8 index, float value)
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[5];

    cmdData[0] = index;
    memcpy(cmdData+1, &value, sizeof(float));

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_SVI_SET_DAC_VALUE, cmdData, sizeof(cmdData)));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);
    return (status == DscpStatus::OK);
}

bool SolenoidValveInterface::ProportionalValve_AutoControlPressure(bool isAuto)
{
    Uint16 status = DscpStatus::Error;
    bool cmdData = isAuto;

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_SVI_SET_AUTO_CONTROL_PRESSURE, &cmdData, sizeof(cmdData)));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);
    return (status == DscpStatus::OK);
}
...
* @return 返回状态，操作是否成功。
 */
bool SolenoidValveInterface::SetValveMapNormalOpen(ValveMap map)
{
    Uint16 status = DscpStatus::Error;
    Uint32 mapVal = map.GetData();

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_SVI_SET_VALVE_MAP_NORMAL_OPEN, &mapVal, sizeof(mapVal)));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);
    return (status == DscpStatus::OK);
}
 ...
 * @details 定时2s向上汇报压力。
 *  上报周期暂不更改。
 * @param[in] handle 定时接收压力上报事件的对象。
 */
void SolenoidValveInterface::RegisterPressureNotice(IEventReceivable *handle)
{
    EventHandler::Instance()->Register(m_addr,DSCP_EVENT_SVI_PREESURE_NOTICE,handle);
}
