...
/**
 * @brief 。
 * @param index Uint8，传感器索引，0注射器索引 1进样电机索引)
 * @return 传感器状态，Uint8,0和1(0为遮挡，1为未遮挡)
 */
Uint8 PeristalticPumpInterface::GetSenseStatus(Uint8 index)
{
    Uint8 statu = 0;

    Uint8 num = (Uint8)index;
    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_PPI_GET_SENSE_STATUS, &num, sizeof(num)));
    SyncCaller syncCaller(m_retries);
    DscpRespPtr resp = syncCaller.Send(cmd);
    if (resp)
    {
        statu = *((Uint8*)resp->data);
    }
...
/**
 * @brief 查询电机的偏移步数。
 * @param[in] index Uint8，要查询的泵索引，0号泵为进样电机。
 * @return 校准系数， float，电机的偏移步数，单位ms。
 * @see DSCP_CMD_PPI_SET_PUMP_OFFSET_STEP
 */
float PeristalticPumpInterface::GetPumpOffsetStep(Uint8 index)
{
    float factor = 0;

    Uint8 num = (Uint8)index;

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_PPI_GET_PUMP_OFFSET_STEP, &num, sizeof(num)));
    SyncCaller syncCaller(m_retries);
    DscpRespPtr resp = syncCaller.Send(cmd);
    if (resp)
    {
 ...
 * @return 返回状态，操作是否成功。
 */
bool PeristalticPumpInterface::SetPumpOffsetStep(Uint8 index, float factor)
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[5];
    ...
    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_PPI_SET_PUMP_OFFSET_STEP, cmdData, sizeof(cmdData)));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);
    return (status == DscpStatus::OK);
