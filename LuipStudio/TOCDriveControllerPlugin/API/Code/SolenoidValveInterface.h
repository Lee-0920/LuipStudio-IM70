 ...
 *  - @ref DSCP_ERROR 操作失败；
 */
#define DSCP_CMD_SVI_SET_DAC_VALUE                  (DSCP_SVI_CBASE + 0x03)
 ...
 *  - @ref DSCP_ERROR 操作失败；
 */
#define DSCP_CMD_SVI_SET_AUTO_CONTROL_PRESSURE       (DSCP_SVI_CBASE + 0x04)

/**
 * @brief 设置常开阀。
 * @return 通道映射图，Uint32，每位表示一个通道的开关状态，1为打开，0为闭合，低位开始。
 */
#define DSCP_CMD_SVI_SET_VALVE_MAP_NORMAL_OPEN       (DSCP_SVI_CBASE + 0x05)

// *******************************************************************
// 事件
/**
 * @brief 上报压力值事件。
 * @param index 通道索引0-2，value 0-500 kPa。
 * @return 状态回应，Uint16，支持的状态有：
 *  - @ref DSCP_OK  操作成功；
 *  - @ref DSCP_ERROR 操作失败；
 */
#define DSCP_EVENT_SVI_PREESURE_NOTICE              (DSCP_SVI_EBASE + 0x00)
