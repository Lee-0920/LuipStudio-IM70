...
TOCDriveController::TOCDriveController(DscpAddress addr)
    ...
    m_upEnvironmentTemp(0),
    m_downEnvironmentTemp(0),
    ...
    m_timeOutLen(0)
{
    ...
    memset(m_exScanData, 0, sizeof(m_exScanData));
    memset(m_MeasureTemp, 0, sizeof(m_MeasureTemp));
    memset(m_EnvTemp, 0, sizeof(m_EnvTemp));
    memset(m_pressure, 0, sizeof(m_pressure));
    memset(m_exMeasureTemp, 0, sizeof(m_exMeasureTemp));
    memset(m_exEnvTemp, 0, sizeof(m_exEnvTemp));
    memset(m_exPressure, 0, sizeof(m_exPressure));
    try
    {
...
MotorControlInterface *TOCDriveController::GetIMotorControl()
{
...
double TOCDriveController::GetExScanData(int index) const
{
    if(index < DATA_MAX_LENGTH)
    {
        return m_exScanData[index];
    }

    return 0;
}

int TOCDriveController::GetExScanLen(void) const
{
    return m_exScanLen;
}

double TOCDriveController::GetExData(void)
{
    if(m_exScanLen)
    {
//        qDebug("Test OK len %d, %.3f", m_exScanLen, m_exScanData[m_exScanLen-1]);
        return  m_exScanData[m_exScanLen-1];
    }
    return 0;
}
...
void TOCDriveController::ClearExBuf(void)
{
    memset(m_exScanData, 0, sizeof(m_exScanData));
    memset(m_exMeasureTemp, 0, sizeof(m_exMeasureTemp));
    memset(m_exEnvTemp, 0, sizeof(m_exEnvTemp));
...
//    measureResultTable.safe_at("filterStep", filterStep);
    ...
    if(isExtra)
    {
         Filter(&m_exScanData[startIndex], &filterbuffer[0], length, filterStep, throwNum);
    }
    else
    {
    ...
    for(int i = 0; i < length; i++) //全部取负，得到正峰
    {
       ...
       if(i > 20+filterStep*2 && i < length - 20-filterStep*2)
       {
           if(debugMode)
           {
               if(isExtra)
               {
                   logger->debug("SFilter[%d]: %f, NDIR: %f , Env: %f, Pre: %f",
                                 i, filterbuffer[i], m_exMeasureTemp[startIndex+i], m_exEnvTemp[startIndex+i], m_exPressure[startIndex+i]);
               }
               else
               {
                   logger->debug("LFilter[%d]: %f, NDIR: %f , Env: %f, Pre: %f",
                                 i, filterbuffer[i], m_MeasureTemp[startIndex+i], m_EnvTemp[startIndex+i], m_pressure[startIndex+i]);
               }
...
bool TOCDriveController::IsReachSteady(int num, int validCnt, int step, int increment, int filterStep, int throwNum, int index, bool isExtra) const
{
    if(isExtra)
    {
        qDebug("q IC");
    ...
    if(isExtra)
    {
        Filter(&m_exScanData[m_exScanLen-length], &filterbuffer[0], length, filterStep, throwNum);
    }
    else
    {
    ...
    if(index == 0)
    {
        ...
        if(isExtra)
        {
            minIndex = m_exScanLen - 1;  //减去数组未装填的指针地址
        }
...
//    qDebug("%d, %f, %f, %f", indexNDIR, measureModuleTemperature, NDIRToleranceTemp, NDIRTempTimeout);
        ...
        //燃烧炉监测
        if((fabs(m_thermostatTempArray[0] - reactTemperature) > 15))
        {
            if(((m_thermostatTempArray[0] - reactTemperature) < -15))//温度过冲后，低温重新打开电炉电源
            {
...
//    logger->debug("\n get event code = %d",event->code);
    switch (event->code)
    {
        ...
        {
            m_thermostatTempArray[0] = *((float*)event->data); //电炉温度
            m_temperature.thermostatTemp = m_thermostatTempArray[0];
            m_thermostatTempArray[1] = *((float*)(event->data+4));//制冷温度
            m_thermostatTempArray[2] = *((float*)(event->data+8));//长测量池温度
            if(event->len >= 12)
            {
                m_thermostatTempArray[3] = *((float*)(event->data+12));//短测量池温度
            }
            if(event->len >= 16)
            {
                m_thermostatTempArray[4] = *((float*)(event->data+16));//测量模块温度
            }
            if(event->len >= 20)
            {
                m_thermostatTempArray[5] = *((float*)(event->data+20));//上机箱温度
                m_upEnvironmentTemp = m_thermostatTempArray[5];
                m_temperature.environmentTemp = m_upEnvironmentTemp;
            }
            if(event->len >= 24)
            {
                m_thermostatTempArray[6] = *((float*)(event->data+24));//下机箱温度
                m_downEnvironmentTemp = m_thermostatTempArray[6];
            }
            if(event->len >= 28)
            {
                m_thermostatTempArray[7] = *((float*)(event->data+28));//预留温度
            }

            emit BoxFanControlSignal(m_upEnvironmentTemp);
            emit ExBoxFanControlSignal(m_downEnvironmentTemp);
            ...
            if(!m_notifise.empty())
            {
                ...
                for(it = m_notifise.begin(); it != m_notifise.end(); it++)
                {
                    (*it)->OnSignalChanged("tTemp", m_thermostatTempArray[0]);
                    (*it)->OnSignalChanged("cTemp", m_thermostatTempArray[1]);
                    (*it)->OnSignalChanged("LMeaTemp", m_thermostatTempArray[2]);
                    (*it)->OnSignalChanged("SMeaTemp", m_thermostatTempArray[3]);
                    (*it)->OnSignalChanged("NDIRTemp", m_thermostatTempArray[4]);
                    (*it)->OnSignalChanged("upTemp", m_thermostatTempArray[5]);
                    (*it)->OnSignalChanged("downTemp", m_thermostatTempArray[6]);
                }
            }
        }
        ...
        {
            double smea = *((Uint32*)(event->data));
            double lmea = *((Uint32*)(event->data + 4));
            double abs = 0;
//            if (0 < ref && 0 < mea)
//            {
...
//            }
            ...
            if(!m_notifise.empty())
            {
                ...
                for(it = m_notifise.begin(); it != m_notifise.end(); it++)
                {
                    (*it)->OnSignalChanged("lmea", lmea);
                    (*it)->OnSignalChanged("smea", smea);
...
//            logger->debug("\n event is DSCP_EVENT_SVI_PREESURE_NOTICE");
        ...
        case DSCP_EVENT_OAI_REPORT_DATA:   // NDIR测量值定时上报事件
        {
            ...
            m_EnvTemp[m_scanLen] = m_upEnvironmentTemp;  //环境温度跟踪
            m_pressure[m_scanLen] = m_pressureArray[2];    //载气压力跟踪
            ...
            break;
        case DSCP_EVENT_OAI_REPORT_SMEA_DATA:   // NDIR测量值定时上报事件
        {
            m_exScanData[m_exScanLen] = *((float*)(event->data));
            m_exMeasureTemp[m_exScanLen] = m_thermostatTempArray[1];  //温度补偿跟踪
            m_exEnvTemp[m_exScanLen] = m_upEnvironmentTemp;  //环境温度跟踪
            m_exPressure[m_exScanLen] = m_pressureArray[1];    //载气压力跟踪
            m_exScanLen++;   //勿在定时器中修改索引
            if(m_exScanLen > DATA_MAX_LENGTH-1)
            {
                m_exScanLen = 0;
                memset(m_exScanData, 0, sizeof(m_exScanData));
                memset(m_exMeasureTemp, 0, sizeof(m_exMeasureTemp));
...
void TOCDriveController::BaseLineStatusMonitor()
{
    ...
    LuaEngine* luaEngine = LuaEngine::Instance();
    lua_State * state = luaEngine->GetThreadState();
    luaEngine->GetLuaValue(state, "config.hardwareConfig.measureLed.period[1].real", LEDPeriod);
...
//    qDebug("num %d, period = %d, %d, %d", dataNum, LEDPeriod, filterStep, throwNum);
    ...
    if(m_scanLen > dataNum)
    {
        ...
        if(true == this->IsReachSteady(dataNum,15,45,step,filterStep,throwNum,0,false))
        {
            ret = true;
        }
        else
